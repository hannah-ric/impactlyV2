  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)]">
      ...
      <header className="fixed top-md left-1/2 -translate-x-1/2 w-[calc(100%-var(--space-lg))] max-w-[1400px] z-sticky animate-slide-down">
        <div className="glass flex items-center justify-between gap-lg p-sm md:p-md rounded-2xl shadow-md relative overflow-hidden">
          {/* Mobile Menu Button */}
          <Button
            variant="ghost"
            size="icon"
          ...
          <div className="hidden md:block flex-1 max-w-[500px] relative">
            <input
              type="search"
              className="w-full h-11 pl-11 pr-[var(--space-xl)] bg-[var(--bg-secondary)] border border-transparent rounded-full text-sm text-[var(--text-primary)] transition-all duration-normal ease-out focus:bg-[var(--bg-primary)] focus:border-primary/30 focus:shadow-[0_0_0_3px_hsla(var(--hue-primary),80%,60%,0.1)] focus:scale-[1.01] hover:bg-[var(--bg-primary)] hover:shadow-sm"
            ...
            <div className="absolute right-md top-1/2 -translate-y-1/2 flex gap-xs pointer-events-none">
              <span className="py-0.5 px-1.5 bg-[var(--neutral-100)] dark:bg-[var(--neutral-800)] border border-[var(--neutral-200)] dark:border-[var(--neutral-700)] rounded-sm text-[11px] font-medium text-[var(--text-tertiary)] shadow-[0_1px_0_0_var(--neutral-200)] dark:shadow-[0_1px_0_0_var(--neutral-700)]">
                ⌘
              </span>
              <span className="py-0.5 px-1.5 bg-[var(--neutral-100)] dark:bg-[var(--neutral-800)] border border-[var(--neutral-200)] dark:border-[var(--neutral-700)] rounded-sm text-[11px] font-medium text-[var(--text-tertiary)] shadow-[0_1px_0_0_var(--neutral-200)] dark:shadow-[0_1px_0_0_var(--neutral-700)]">
                K
      ...
      {/* Sidebar */}
      <nav
        className={`fixed left-md top-[calc(64px+var(--space-md)*2)] bottom-md w-[260px] bg-[var(--bg-elevated)] rounded-xl shadow-md overflow-hidden z-elevated flex flex-col animate-slide-in-left ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"} transition-transform duration-normal ease-out`}
      >
